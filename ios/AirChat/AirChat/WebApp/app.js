...
const translations = {
    ro: {
        ...
        download_apk: "Vrei aplicația nativă? Descarc-o de aici"
    },
    en: {
        ...
        m_suffix: " m",
        online_count: "{count} online",
        download_apk: "Want the native app? Download it here"
    }
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Skipped in the native host (iOS app): those files come straight out of the
    // app bundle and this page *is* the server, so a cache only risks serving a
    // stale bundle after an app update. Guests connecting over Wi-Fi still get it.
    if ('serviceWorker' in navigator && !window.isAirChatNativeHost) {
    ...
    if (photoBtn && fileInput) {
        photoBtn.addEventListener('click', () => {
            if (window.isAirChatNativeHost && window.nativePickImage) { window.nativePickImage(); return; }
            fileInput.click();
        });
...
function crashAndCancel(x, y) {
    ...
    if (window.isAirChatNativeHost && window.nativeBuzz) { window.nativeBuzz(); }
...
async function startRecording() {
    ...
    // The iOS host app records natively: WKWebView only exposes getUserMedia in a
    // secure context, and http://192.168.x.x is not one. See airchat-bridge.js.
    if (window.isAirChatNativeHost && window.nativeStartRecording) {
        window.nativeStartRecording();
...
function stopRecording() {
    isRecordingRequested = false;
    if (window.isAirChatNativeHost && window.nativeStopRecording) { window.nativeStopRecording(); return; }
...
function cancelRecording() {
    isRecordingRequested = false;
    if (window.isAirChatNativeHost && window.nativeCancelRecording) { window.nativeCancelRecording(); return; }
    if (!isRecording) return;
    isRecordingCancelled = true;
...
// ==============================
// === NATIVE HOST (iOS) UTILS ===
// ==============================
// On iOS the microphone, the photo picker and the haptics belong to the host app:
// WKWebView only exposes getUserMedia on secure origins (http://192.168.x.x is not
// one) and has no <input type="file"> support whatsoever. The transport lives in
// airchat-bridge.js; the state machine stays here so both platforms share one chat.
let nativeRecordingArmed = false;
let nativeRecordingPending = false;

function nativeAvailable() { return !!(window.AirChatNative && window.AirChatNative.available); }
function nativeCall(action) { if (nativeAvailable()) return window.AirChatNative.call({ action: action }); return Promise.resolve(null); }

function nativeBuzz() { nativeCall('buzz'); }
function nativePickImage() { nativeCall('pickImage'); }

async function nativeStartRecording() {
    nativeRecordingArmed = true;
    nativeRecordingPending = true;
    const ok = await nativeCall('recordingStart');
    nativeRecordingPending = false;
    // The finger may have left the button while the OS asked for mic permission.
    if (!nativeRecordingArmed) { nativeCall('recordingCancel'); return; }
    if (!ok) { alert(t("alert_mic")); return; }
    isRecording = true;
...
function nativeStopRecording() {
    if (nativeRecordingPending) { nativeRecordingArmed = false; return; }
    if (!nativeRecordingArmed && !isRecording) return;
    nativeRecordingArmed = false;
    const tooShort = (Date.now() - recordingStartTime) < 400;
    if (tooShort || isRecordingCancelled) {
        nativeCall('recordingCancel');
        resetRecordingUI();
        return;
    }
    nativeCall('recordingStop');       // the bytes come back through onAudio below
    resetRecordingUI();
}

function nativeCancelRecording() {
    nativeRecordingArmed = false;
    if (nativeRecordingPending) return;
    nativeCall('recordingCancel');
    resetRecordingUI();
}

window.nativeBuzz = nativeBuzz;
window.nativePickImage = nativePickImage;
window.nativeStartRecording = nativeStartRecording;
window.nativeStopRecording = nativeStopRecording;
window.nativeCancelRecording = nativeCancelRecording;

if (nativeAvailable()) {
    window.AirChatNative.onAudio = function (dataUrl) {
        if (!dataUrl) return;
        sendPayload({ type: 'audio', id: generateId(), audio: dataUrl });
    };
    window.AirChatNative.onImage = function (dataUrl) {
        if (!dataUrl) return;
