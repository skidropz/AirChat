...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (disconnectBtn) {
        disconnectBtn.addEventListener('click', () => {
            // Reîncărcarea paginii este cea mai simplă formă de reinițializare/logout într-o aplicație SPA
...
function openImage(src) {
    ...
    if (overlay && fullImg) { fullImg.src = src; overlay.classList.add('active'); }
}
...
function handleData(data) {
    if (data.sender && data.sender !== myName) {
        if (!knownUsers.has(data.sender)) { knownUsers.set(data.sender, data.color || "blue"); updateOnlineCounter(); }
        else { knownUsers.set(data.sender, data.color || "blue"); }
    }
...
function sendSeen(msgId) {
    if(!msgId) return;
    sendPayload({ type: 'seen', seenMsgId: msgId, seenBy: myName });
}

function handleSeenEvent(data) {
    const msgId = data.seenMsgId;
    const who = data.seenBy;
    if(!msgId || !who || who === myName) return;
    ...
    if(msgRow) {
        ...
        if(!seenLabel) {
            ...
            msgRow.appendChild(seenLabel);
        }
        const currentText = seenLabel.textContent;
        if(currentText.includes(who)) return;
...
function attachSwipeListeners(element, messageData) {
    ...
    element.addEventListener('touchend', () => {
        const diff = currentX - startX; element.style.transform = 'translateX(0)';
        if (diff > 50) triggerReply(messageData);
        startX = 0; currentX = 0;
    });
...
function triggerReply(messageData) {
    const replyText = messageData.type === 'image' ? '[Imagine]' : messageData.text;
    replyingTo = { id: messageData.id, sender: messageData.sender, text: replyText };
    document.getElementById('reply-bar').classList.remove('hidden');
    document.getElementById('reply-target-name').textContent = messageData.sender;
    document.getElementById('reply-target-text').textContent = replyText;
...
function renderMessage(data) {
    ...
    if (data.replyTo) {
        const origId = data.replyTo.id;
        content += `<div class="reply-preview-in-message" onclick="scrollToMessage('${origId}')">
                        <span class="reply-from-name">${data.replyTo.sender}</span>
                        <span class="reply-original-text">${data.replyTo.text}</span>
                    </div>`;
    }
    ...
    if (!isMe) {
        if (document.hidden) { unseenMsgQueue.add(data.id); } else { sendSeen(data.id); }
    }
}
...
function renderImageMessage(data) {
    ...
    content += `<img src="${data.image}" class="chat-image" alt="Imagine" onclick="openImage('${data.image}')">`;
    content += `</div>`;
    row.innerHTML = content;
    list.appendChild(row);
    requestAnimationFrame(() => { list.scrollTop = list.scrollHeight; });
